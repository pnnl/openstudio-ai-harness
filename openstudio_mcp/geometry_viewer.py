...
function draw(){{ctx.clearRect(0,0,canvas.clientWidth,canvas.clientHeight);let faces=scene.faces.filter(visible).map(f=>({{f,p:f.vertices.map(project)}})).sort((a,b)=>b.p.reduce((s,x)=>s+x[2],0)-a.p.reduce((s,x)=>s+x[2],0));for(const item of faces){{let p=item.p;ctx.beginPath();ctx.moveTo(p[0][0],p[0][1]);for(let i=1;i<p.length;i++)ctx.lineTo(p[i][0],p[i][1]);ctx.closePath();let f=item.f;ctx.fillStyle=f.space_id===selected?'#ef5b5b':(colors[f.kind==='shading'?'shading':f.surface_type]||colors.default);ctx.globalAlpha=f.kind==='subsurface'?.92:.62;ctx.fill();ctx.globalAlpha=1;ctx.strokeStyle=f.space_id===selected?'#9b1c1c':'#ffffffaa';ctx.lineWidth=f.space_id===selected?2:1;ctx.stroke()}}}}
...
function renderedFaces(){{return scene.faces.filter(face=>visible(face)).map(face=>({{face:face,points:face.vertices.map(project)}})).sort((a,b)=>b.points.reduce((sum,p)=>sum+p[2],0)-a.points.reduce((sum,p)=>sum+p[2],0))}}
