...
def _render_install_doc(
    ...
    return (
        ...
        "* **OpenStudio 3.11+** installed and visible on your PATH\n"
        "* The **`openstudio-ai`** Python package installed\n"
        "* **Codex** installed and functioning\n\n"
        "## OpenStudio PATH Setup\n\n"
        "The OpenStudio AI runtime checks `OPENSTUDIO_PATH` first, then searches "
        "the PATH used to launch Codex for the `openstudio` executable. OpenStudio "
        "is not bundled with the plugin.\n\n"
        "Add OpenStudio's `bin` directory to your PATH so Codex can find it:\n\n"
        "* **Windows:** Add `C:\\openstudio-3.11.0\\bin` to your system PATH via "
        "System Properties → Environment Variables.\n"
        "* **macOS:** Add `export PATH=\"/Applications/OpenStudio-3.11.0/bin:$PATH\"` "
        "to your `~/.zshrc` or `~/.bash_profile`.\n"
        "* **Linux:** Add `export PATH=\"/usr/local/openstudio-3.11.0/bin:$PATH\"` "
        "to your `~/.bashrc`.\n\n"
        "Adjust the version number to match your installed version. After updating "
        "PATH, restart your terminal and Codex.\n\n"
        "Alternatively, set `OPENSTUDIO_PATH` to the full path of the OpenStudio "
        "executable in the plugin's `.mcp.json` `env` block — the runtime checks "
        "this variable before searching PATH.\n\n"
        "## (Optional, for Developers) Validate The Plugin\n\n"
        "> This step is optional and only relevant if you're developing or testing "
        "the plugin itself. It requires access to the separate "
    ...
    return {
        "setup-openstudio-ai": _skill_markdown(
            ...
            body=(
                ...
                "6. If the runtime is missing, explain the issue in normal energy-modeler "
                ...
                "12. Summarize readiness for model loading, HVAC workflow support, simulation, "
